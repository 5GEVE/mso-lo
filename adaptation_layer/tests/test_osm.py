...
class OSMTestCase(unittest.TestCase):
    ...
    def test_get_ns_list_200(self):
        res = self.client().get('/nfvo/nfvo_osm1/ns_instances?__code=200')
        self.assertEqual(res.status_code, 200)

    def test_get_ns_list_401(self):
        res = self.client().get('/nfvo/nfvo_osm1/ns_instances?__code=401')
        self.assertEqual(res.status_code, 401)

    # Check status codes 200, 401, 404, headers and payload for get_ns()
    def test_get_ns_200(self):
        res = self.client().get('/nfvo/nfvo_osm1/ns_instances/ns_id_1?__code=200')
        self.assertEqual(res.status_code, 200)

    def test_get_ns_404(self):
        res = self.client().get('/nfvo/nfvo_osm1/ns_instances/ns_id_1_missing?__code=404')
        self.assertEqual(res.status_code, 404)

    def test_get_ns_401(self):
        res = self.client().get('/nfvo/nfvo_osm1/ns_instances/ns_id_1?__code=401')
        self.assertEqual(res.status_code, 401)

    # Check status codes 201, 401, 404, headers and payload for create_ns()
    def test_create_ns_201(self):
        res = self.client().post('/nfvo/nfvo_osm1/ns_instances?__code=201', json=self.mock_ns)
        self.assertEqual(res.status_code, 201)

    def test_create_ns_400(self):
        res = self.client().post('/nfvo/nfvo_osm1/ns_instances?__code=400', json=self.mock_ns)
        self.assertEqual(res.status_code, 400)

    def test_create_ns_401(self):
        res = self.client().post('/nfvo/nfvo_osm1/ns_instances?__code=401', json=self.mock_ns)
        self.assertEqual(res.status_code, 401)
